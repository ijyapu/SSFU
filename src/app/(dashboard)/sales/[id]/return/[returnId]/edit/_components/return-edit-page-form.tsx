...
export function ReturnEditPageForm({
  soId,
  returnData,
  products,
}: {
  ...
  const isFresh = returnData.returnType === "FRESH";
  ...
  async function handleSubmit() {
    ...
    setLoading(true);
    try {
      await updateSalesReturn(returnData.id, { items, notes: notes.trim() || undefined });
  ...
  const accentColor  = isFresh ? "text-green-600"  : "text-orange-600";
  const accentTotal  = isFresh ? "text-green-700"  : "text-orange-700";
            ...
            return (
              ...
              >
                ...
                <div className="px-3 py-2.5 flex items-center justify-end min-h-11">
                  {lineTotal > 0
                    ? <span className={`text-sm tabular-nums font-semibold ${accentTotal}`}>{fmt(lineTotal)}</span>
      ...
      <div className="w-full xl:w-80 xl:shrink-0 rounded-lg border bg-muted/20 flex flex-col xl:sticky xl:top-6">
        ...
        <div className="p-5 space-y-4">
          ...
          <div className="flex items-start justify-between gap-2">
            ...
            <span className={`text-xl font-bold tabular-nums leading-tight ${total > 0 ? accentTotal : "text-muted-foreground/30"}`}>
              {total > 0 ? `Rs ${fmt(total)}` : "—"}
            </span>
          </div>
          {isFresh && total > 0 && (
            <p className="text-[11px] text-blue-700 dark:text-blue-400 leading-relaxed">
              Stock will be adjusted: old quantities reversed, new quantities restocked.
            </p>
          )}
          {!isFresh && total > 0 && (
        ...
        <div className="p-5 space-y-2">
          ...
          <Textarea
            ...
            rows={4}
            placeholder="e.g. expired on 2025-04-14, packaging damaged..."
        ...
        <div className="p-5 space-y-3">
          ...
          <Button
            className={`w-full gap-2 ${accentBtn}`}
